"""
Statewide jurisdiction creation script. This script runs through all the sub-directories in the DATA_PATH and assumes those are counties.
It then runs through any sub-directories in each county and assumes those are cities. It then uses the boundary json in those city directories
to create individual jurisdiction priority and feasability jsons.

Input data structure:
---------------------
DATA_PATH->[county_dirs]->[city_dirs]->boundary.geojson

The included config file needs to contain all the priority and feasability attributes to be joined, but leaves out the jurisdiction section.

The OUTPUT_PATH needs to have the same counties/cities directory structure as the input data directory. The finished priority/feasability jsons
will be placed into these directories.

Output data structure:
----------------------
OUTPUT_PATH->[county_dirs]->[city_dirs]->[priority/feasability.jsons]
...
def run(config):
    ...
    for c in county_dirs:
        ...
        for jurisdiction in jurisdictions:
            ...
            jbfile = os.path.join(jurisdiction, str(os.path.basename(os.path.normpath(jurisdiction)) + '_city_boundary.geojson'))
            jbounds[jname] = gpd.read_file(jbfile)
        ...
        for pixel_type in ['priority', 'feasibility']:
            ...
            for k in pixel_dfs[pixel_type].keys():
                ...
                outfile = "{}/{}/{}/{}_{}.json".format(OUTPUT_PATH, os.path.basename(os.path.normpath(c)), k, k, pixel_type)
